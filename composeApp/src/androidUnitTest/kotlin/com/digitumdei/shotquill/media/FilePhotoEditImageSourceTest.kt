...
class FilePhotoEditImageSourceTest {
    private val tmpDir = createTempDir("shotquill-photoedit-source-test-")
    private val source = FilePhotoEditImageSource()
    ...
    fun `load succeeds for valid file URI with existing file`() {
        ...
        assertEquals("test_photo.jpg", image.fileName)
    }

    @Test
    fun `load returns Failure for content URI scheme`() {
        val asset = MediaAsset(
            id = MediaAssetId("photo-src-2"),
            type = MediaType.Photo,
